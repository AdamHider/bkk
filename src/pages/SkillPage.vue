<template>
  <q-page>
    <div v-if="skill.id">
      <div class="column no-wrap" style="min-height: 100%;">
    ...
    <q-dialog v-model="helpDialog" position="bottom">
      <SkillHelpSheet />
    </q-dialog>
  </q-page>
</template>
...
import SkillHelpSheet from '../components/SkillHelpSheet.vue'
...
const loadSkill = async () => {
  try {
        const response = await api.post('/Skill/getItem', { child_id: 1, skill_id: route.params.skill_id })
...
const activeStepIndex = computed(() => {
  const index = skill.value.stages.findIndex(s => !s.is_completed)
  return index === -1 ? skill.value.stages.length - 1 : index
})
